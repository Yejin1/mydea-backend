...
class OrderControllerTest {
        ...
        @Test
        @DisplayName("POST /api/orders/direct - 바로 주문 성공")
        void direct_ok() throws Exception {
                DirectOrderRequest req = new DirectOrderRequest();
                req.setItems(List.of(
                                new com.mydea.mydea_backend.cart.dto.AddItemReq(10L, "opt10", "Work10",
                                                "https://img/10.png", 10000, 2)));
                req.setRecipientName("홍길동");
                req.setPhone("01012345678");
                req.setAddress1("서울시 어딘가");
                req.setAddress2("101동");
                ...
                OrderResponse resp = response(101L, OrderStatus.PAYMENT_PENDING, List.of(
                                item(1L, 10L, 10000, 2)));

                // virtual cartId stub
                given(cartService.createVirtualCart(eq(1L), anyList(), anyLong())).willReturn(9999L);
                given(orderService.create(eq(1L), isNull(), any(OrderCreateRequest.class))).willReturn(resp);

                mvc.perform(post("/api/orders/direct")
                                .with(csrf())
                                .with(user("1"))
                                .principal(new UsernamePasswordAuthenticationToken("1", null, Collections.emptyList()))
                                .contentType(MediaType.APPLICATION_JSON)
                ...
                then(cartService).should().createVirtualCart(eq(1L), anyList(), anyLong());
                then(orderService).should().create(eq(1L), isNull(), any(OrderCreateRequest.class));
        }
